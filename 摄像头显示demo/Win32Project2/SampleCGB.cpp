...
//链接两个Filter
HRESULT ISampleCaptureGraphBuilder::ConnectFilters(IBaseFilter *pUpFilter, IBaseFilter *pDownFilter)
{
	...
	while(pUpFilterPin.Release( ), S_OK == pEnumUpFilterPins->Next(1, &pUpFilterPin, &nFetched) )
	{
		...
		if( FAILED( hr ) || PINDIR_INPUT == pinDir )				//上游filter的输出pin
		{
			continue;
		}
		...
		while(pDownFilterPin.Release( ), S_OK == pEnumDownFilterPins->Next(1, &pDownFilterPin, &nFetched) )
		{
			...
			if( FAILED( hr ) || PINDIR_OUTPUT == pinDir )			//下游filter的输入pin
			{
				continue;
			}
...
HRESULT ISampleCaptureGraphBuilder::ConnectPin( IPin *pPin, IBaseFilter *pDownFilter )
{
	if( !pPin || !pDownFilter )
	{
		return E_INVALIDARG;
	...
	if( FAILED( hr ) || PINDIR_INPUT == pinDirection )			//pinDirection必须为输出pin
	{
		return E_FAIL;
	}
	...
	if( !bConnected )
	{
		graph_->RemoveFilter( pDownFilter );				//pin和pDownFilter链接不上，删除这个filter
		return E_FAIL;
	}
